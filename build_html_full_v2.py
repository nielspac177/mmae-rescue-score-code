...
<div class="lede">
Among 214 consecutive MMA embolization procedures for chronic subdural hematoma, 36 (16.8%) led to rescue surgery. We built three integer risk scores plus a parallel data-driven model. The full 8-point Model 1 reaches AUC 0.69 (optimism-corrected); Model 3 (6-point) 0.68; the simplified 5-point Model 2 0.62; and a lasso-selected data-driven Model 4 reached cross-validated AUC 0.67 (95% CI 0.58–0.76). A Model 1 score of ≥5 separates a 10.4% rescue cohort from a 38.0% rescue cohort, a 3.7-fold difference. SDH volume ≥100 mL and age are the two strongest individual contributors.
</div>
...
<p>Model 1 (range 0–8) gives 0, 1, or 2 points for age &lt;65, 65–80, and &gt;80 years, plus 1 point each for SDH volume ≥100 mL, anticoagulation, focal neurological deficit at presentation, platelets &lt;150 ×10⁹/L, antiplatelet therapy, and embolization of both anterior and posterior branches. Model 3 (range 0–6) is a five-variable variant with the age cutoff at &gt;85 instead of &gt;80, retaining laboratory, antiplatelet, and focal-deficit predictors. Model 2 (range 0–5) is a triage-friendly version using age, SDH volume ≥100 mL, anticoagulation, and focal deficit. After all clinical scores were derived, we built a parallel data-driven Model 4 by lasso-selected logistic regression on the full candidate-variable matrix (25 predictors), capping non-zero coefficients at the 1-in-5 events-per-variable maximum of 7. Lasso selected age, platelets, hypertension, antiplatelet, gait disturbance, anterior + posterior embolization, and SDH volume ≥100 mL.</p>
<p>An a-priori coding error in the focal-deficit variable was detected during data-quality review and corrected before final analysis. The corrected prevalence (128/214, 59.8%) is consistent with published MMA-embolization series (40–70%), whereas the prior coding (56/214, 26.2%) was anomalously low. The correction did not alter any other variable. Focal deficit was retained in Models 1, 2, and 3 for clinical face validity, although it was non-significant in the multivariable Model 1 (adjusted OR 0.80, 95% CI 0.36–1.78; P = 0.58), the AIC was 1.7 units higher with it than without, and the data-driven Model 4 did not retain it.</p>
...
<p>Rescue rates climbed stepwise with the full Model 1 score: 0–15% across scores 0–4, then 41.0% at score 5, 22.2% at 6, and 50.0% at score 7. The break is between scores 4 and 5. Dichotomizing at ≥5 produced a low-risk arm of 164 patients with a 10.4% rescue rate (17/164) and a high-risk arm of 50 patients with a 38.0% rate (19/50), a 3.7-fold difference.</p>
{img("fig2_score_risk")}
...
<h2>4 · Discussion</h2>
<p>We built and internally validated three pre-procedural integer scores plus one data-driven logistic model for the rescue-surgery endpoint after MMA embolization. The full 8-point Model 1 discriminates rescue with an optimism-corrected AUC of 0.69 and separates a low-risk arm (10.4% rescue rate) from a high-risk arm (38.0%) at the cutoff of ≥5, a 3.7-fold difference. Model 3 reaches a comparable AUC of 0.68 with five variables. The simplified 5-point Model 2 lands at AUC 0.62 and is useful when laboratory or procedural data are not yet available. The data-driven Model 4 reached an essentially identical 5×10 cross-validated AUC of 0.67 (95% CI 0.58–0.76), confirming the integer scores are not sacrificing discrimination for parsimony. All three integer scores are calibrated across deciles of predicted probability, so the absolute risk numbers can be used directly, not just as a ranking.</p>
...
<p>Focal neurological deficit at presentation is part of every published MMA-embolization predictor list and is consistently associated with worse outcomes in the burr-hole literature. During data-quality review of an earlier draft of this analysis, we identified a coding error in the focal-deficit variable: prevalence had been recorded as 26.2%, substantially below the 40–70% reported in major MMA series. The corrected coding raised prevalence to 59.8%, fully within the published range, and was applied uniformly across the cohort before final model fitting. After the correction, focal deficit was retained in Models 1, 2, and 3 for clinical face validity; however, the variable was non-significant in the multivariable analysis (Model 1 adjusted OR 0.80, 95% CI 0.36–1.78; P = 0.58), the AIC was 1.7 units higher with the variable than without, and the data-driven Model 4 did not retain it at the lasso-selected penalty. Cross-validated AUC differed by less than 0.02 between focal-deficit-included and focal-deficit-removed scores. The likeliest explanation is that focal deficit, while clinically meaningful at the bedside, is heavily collinear with SDH volume, midline shift, and pre-procedure mRS in this cohort and contributes little independent information. We kept the variable in the integer scores because dropping it would force the user of the calculator to ignore a clinical sign they will inevitably observe, and because external validation in a multicenter cohort with consistent indication criteria may yet recover an independent signal.</p>
...
<h2>5 · Conclusion</h2>
<p>A simple 8-point pre-procedural score discriminates rescue surgery after MMA embolization for chronic subdural hematoma with an optimism-corrected AUC of 0.69. A score of ≥5 identifies a patient cohort with a 3.7-fold higher rescue rate than the rest of the population. SDH volume ≥100 mL and age are the two strongest individual contributors. A parallel data-driven Model 4 reaches the same cross-validated AUC as the integer scores, confirming that simplicity is not costing accuracy at this event count. External validation is required before clinical adoption.</p>
