...
<div class="lede">
Among 214 consecutive MMA embolization procedures for chronic subdural hematoma, 36 (16.8%) led to rescue surgery. We built two primary knowledge-driven integer risk scores plus four sensitivity models. The primary 7-point Model 1 reaches AUC 0.70 (optimism-corrected); the simplified 4-point Model 2 reaches AUC 0.64. Sensitivity models: Model 3 (5-point co-author derivation) AUC 0.70; data-driven Model 4 (lasso, EPV-5 cap, three class-imbalance modes) cross-validated AUC 0.66 (95% CI 0.56–0.75); enhanced Model 5 (splines, interactions, tuned elastic-net, stacked ensemble, Bayesian logistic with informative priors anchored to published MMA series, MICE imputation, and a richer continuous-feature space) cross-validated AUC 0.63 (0.52–0.74) for the composite, with the Bayesian-with-priors variant reaching 0.67 (0.57–0.77). A Model 1 score of ≥4 separates an 8.6% rescue cohort from a 36.5% rescue cohort, a 4.2-fold difference. SDH volume ≥100 mL and age are the two strongest individual contributors.
</div>
...
<p><b>Primary models.</b> Model 1 (range 0–7) gives 0, 1, or 2 points for age &lt;65, 65–80, and &gt;80 years, plus 1 point each for SDH volume ≥100 mL, anticoagulation, platelets &lt;150 ×10⁹/L, antiplatelet therapy, and embolization of both anterior and posterior branches. Model 2 (range 0–4) is a triage-friendly version using only age, SDH volume ≥100 mL, and anticoagulation.</p>
<p><b>Sensitivity analyses.</b> Four sensitivity analyses were prespecified. (i) Model 3 (range 0–5) is a co-author parallel derivation using age stratified at &gt;85 instead of &gt;80, SDH volume ≥100 mL, platelets &lt;150 ×10⁹/L, and antiplatelet therapy. (ii) Focal-deficit-included variants of Models 1, 2, and 3 (max 8 / 5 / 6) were retained as a sensitivity sub-analysis after a data-coding correction in the focal-deficit variable; the corrected prevalence (128/214, 59.8%) is consistent with published MMA series (40–70%), whereas the prior coding (56/214, 26.2%) was anomalously low. After correction the variable was non-significant in univariable (OR 1.07, P = 0.86) and multivariable (adjusted OR 0.80, P = 0.58) analyses, and was therefore excluded from primary scores for parsimony. (iii) A data-driven Model 4 was derived independently by L1-penalized logistic regression on the full candidate-variable matrix (25 predictors after near-zero-variance and collinearity filtering), capped at the events-per-variable maximum of 7. Three class-imbalance modes were evaluated: natural (no weighting), class_weight='balanced', and SMOTE applied within each training fold only. The natural lasso selected age, platelets, hypertension, antiplatelet, and SDH volume ≥100 mL. (iv) An enhanced sensitivity Model 5 bundled seven methodological refinements: restricted cubic splines on continuous predictors; prespecified clinical interactions (age × SDH volume, anticoag × low platelets, antiplatelet × age); Optuna-tuned elastic-net (40 trials over C and l1_ratio); stacked ensemble (logistic + tuned gradient boosting with logistic meta-learner); Bayesian logistic regression in PyMC with Normal priors on log-odds ratios anchored to published MMA-embolization series (anticoag OR ≈ 2.0, SDH ≥ 100 mL OR ≈ 2.0, antiplatelet OR ≈ 1.7, midline shift ≥ 5 mm OR ≈ 2.0, age per category OR ≈ 1.5, platelets &lt; 150 OR ≈ 1.8; weakly informative N(0,1) priors otherwise); MICE iterative-chained-equations imputation for missing baseline SDH volumes (15.0% missing); and a richer feature space including continuous mRS, GCS, midline shift in mm, and INR alongside their binarized forms. All approaches were evaluated under the same 5×10 stratified cross-validation. Cross-validated AUCs were 0.61 (95% CI 0.50–0.71) for the enriched lasso, 0.60 (0.50–0.71) for the tuned elastic-net, 0.54 (0.42–0.64) for the stacked ensemble, and <b>0.67 (0.57–0.77) for the Bayesian informative-prior model</b>. The composite mean across the four enhancements gave AUC 0.63 (0.52–0.74). None of the enhancements moved AUC outside the Model-4 confidence interval; the naive feature expansions actually under-performed Model 4, the expected signature of overfitting at this event count.</p>
...
<p>The 214 patients had a mean age of 73.4 years (20.6% &lt;65, 47.7% 65–80, 31.8% &gt;80). One in four (25.7%) was on anticoagulation and just over a third (36.4%) on antiplatelet therapy. SDH volume reached ≥100 mL in 29.0%, platelets fell below 150 ×10⁹/L in 20.6%, and 59.8% had a focal neurological deficit at the time of embolization (after the data-coding correction described in Methods). Anterior and posterior branches were both embolized in 53.7%. Thirty-six patients (16.8%) went on to rescue surgery.</p>
<p><b>Table 1.</b> Baseline characteristics by rescue status.</p>
{df_table(table1)}

<h3>Univariable associations</h3>
<p>On univariable analysis, SDH volume ≥100 mL was the only score variable to reach statistical significance (OR 2.30, 95% CI 1.10–4.80; P = 0.027). Antiplatelet therapy was borderline (OR 1.97, 95% CI 0.95–4.05; P = 0.067). The remaining score variables — age &gt;80 (OR 1.68), platelets &lt;150 (OR 1.93), anticoagulation (OR 1.57), and dual-branch embolization (OR 1.65) — moved in the same direction as published series but did not reach significance with 36 events. Focal neurological deficit was non-significant after the data-coding correction (OR 1.07, 95% CI 0.51–2.22; P = 0.86).</p>
{img("fig4_forest")}
...
<h2>4 · Discussion</h2>
<p>We built and internally validated two pre-procedural knowledge-driven integer scores as the primary models, with three sensitivity models supporting them, for the rescue-surgery endpoint after MMA embolization. The primary 7-point Model 1 discriminates rescue with an optimism-corrected AUC of 0.70 and separates a low-risk arm (8.6% rescue rate) from a high-risk arm (36.5%) at the cutoff of ≥4, a 4.2-fold difference. The simplified 4-point Model 2 lands at AUC 0.64 and is useful when laboratory or procedural data are not yet available. Among sensitivity analyses, the co-author-derived Model 3 reaches a comparable AUC of 0.70; the data-driven Model 4 — derived independently by lasso, with three class-imbalance modes evaluated — reached an essentially identical 5×10 cross-validated AUC of 0.66 (95% CI 0.56–0.75), confirming the integer scores are not sacrificing discrimination for parsimony. Both primary scores are calibrated across deciles of predicted probability, so the absolute risk numbers can be used directly, not just as a ranking.</p>
...
<p>Focal neurological deficit at presentation is part of every published MMA-embolization predictor list and is consistently associated with worse outcomes in the burr-hole literature. During data-quality review of an earlier draft of this analysis, we identified a coding error in the focal-deficit variable: prevalence had been recorded as 26.2%, substantially below the 40–70% reported in major MMA series. The corrected coding raised prevalence to 59.8%, fully within the published range, and was applied uniformly across the cohort. After the correction, focal deficit was non-significant in univariable (OR 1.07, 95% CI 0.51–2.22; P = 0.86) and multivariable (Model 1 adjusted OR 0.80, 95% CI 0.36–1.78; P = 0.58) analyses, the AIC was 1.7 units higher with the variable than without, and the lasso-based Model 4 did not retain it. Cross-validated AUC differed by less than 0.02 between focal-deficit-included and focal-deficit-removed scores. The likeliest explanation is that focal deficit, while clinically meaningful at the bedside, is heavily collinear with SDH volume, midline shift, and pre-procedure mRS in this cohort and contributes little independent information. The variable was therefore excluded from the primary scores for parsimony; focal-included variants of Models 1–3 are reported as a sensitivity sub-analysis to allow direct comparison.</p>
...
<p>Our cohort reflects a single institution's evolving practice in 2024–2026, during the same period in which EMBOLISE,<sup>26</sup> MAGIC-MT,<sup>27</sup> and EMPROTECT<sup>17</sup> shifted clinical equipoise; the score's coefficients should be re-estimated as patient selection patterns mature. Prospective external validation, ideally pooled across the active MMA registries and embedded inside the next generation of randomized trials,<sup>7,25</sup> is the next step.</p>

<h2>5 · Conclusion</h2>
<p>A simple 7-point pre-procedural knowledge-driven score discriminates rescue surgery after MMA embolization for chronic subdural hematoma with an optimism-corrected AUC of 0.70. A score of ≥4 identifies a patient cohort with a 4.2-fold higher rescue rate than the rest of the population. SDH volume ≥100 mL and age are the two strongest individual contributors. Data-driven sensitivity models — lasso (Model 4) and an enhanced bundle including splines, interactions, tuned elastic-net, stacking, and Bayesian logistic with informative priors anchored to published MMA series (Model 5) — reached cross-validated AUCs of 0.66 and 0.67 respectively, confirming that the parsimonious knowledge-driven form is not sacrificing accuracy at this event count. External validation is required before clinical adoption.</p>
