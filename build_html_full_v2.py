...
<div class="lede">
Among 214 consecutive MMA embolization procedures for chronic subdural hematoma, 36 (16.8%) led to rescue surgery. We built two integer risk scores. The full 8-point version reaches AUC 0.73 (optimism-corrected); a trimmed 5-point version reaches 0.68. A score of ≥5 separates a 9.1% rescue cohort from a 42.9% rescue cohort — a 4.7-fold difference. Asymptomatic patients with anatomically large hematomas were the harder group to control.
</div>
...
<p>Two predictors were significant on their own: SDH volume ≥100 mL (OR 2.30, 95% CI 1.10–4.80; P = 0.027) and absence of focal deficit (OR 3.30, 95% CI 1.11–9.80; P = 0.031). Antiplatelet therapy was borderline (OR 1.97, 95% CI 0.95–4.05; P = 0.067). The remaining variables — age &gt;80 (OR 1.68), platelets &lt;150 (OR 1.93), anticoagulation (OR 1.57), and dual-branch embolization (OR 1.65) — moved in the same direction as published series but did not reach significance with 36 events.</p>
{img("fig4_forest")}
...
<p>Rescue rates climbed stepwise with the full score: 0–9% across scores 0–3, then 11.9% at score 4, 42.1% at 5, 37.5% at 6, and 66.7% at 7. The break sits cleanly between 4 and 5. Dichotomizing at ≥5 produced a low-risk arm of 165 patients with a 9.1% rescue rate (15/165) and a high-risk arm of 49 patients with a 42.9% rate (21/49) — a 4.7-fold difference.</p>
{img("fig2_score_risk")}
...
<p>The second finding is counterintuitive and warrants careful interpretation. Patients who came to embolization without a focal neurological deficit had three times the rescue rate of patients who did. Read at face value, this seems wrong: a focal deficit should mark a sicker patient. The most likely explanation is treatment selection rather than biology. A patient with a 120 mL hematoma and a hemiparesis goes to the operating room first and never enters the embolization cohort; a patient with the same hematoma and no deficit gets embolized and watched, then comes back if the hematoma fails to resolve. The asymptomatic-but-radiographically-large group is the residual that flows into MMA embolization in current practice, and that group is where embolization underperforms. This reading aligns with the treatment-effect-heterogeneity analysis recently reported by Chen and colleagues.<sup>18</sup> Until the variable is independently replicated in a cohort with consistent indication criteria, the focal-deficit term should be treated as an indicator of indication for embolization rather than as a biological risk factor for rescue.</p>
...
<p>Two further caveats deserve specific mention. The absence of focal neurological deficit appears as a strong positive predictor of rescue, but as discussed above it almost certainly captures who got selected for embolization rather than a biological risk factor; it should be treated as an indication-marker until external replication. Centers with stricter indication criteria for MMA embolization may not see the same association. Our cohort also reflects a single institution's evolving practice in 2024–2026, during the same period in which EMBOLISE,<sup>26</sup> MAGIC-MT,<sup>27</sup> and EMPROTECT<sup>17</sup> shifted clinical equipoise; the score's coefficients should be re-estimated as patient selection patterns mature. Prospective external validation, ideally pooled across the active MMA registries and embedded inside the next generation of randomized trials,<sup>7,25</sup> is the next step.</p>

<h2>5 · Conclusion</h2>
<p>A simple 8-point pre-procedural score discriminates rescue surgery after MMA embolization for chronic subdural hematoma with an optimism-corrected AUC of 0.73. A score of ≥5 identifies a patient cohort with a 4.7-fold higher rescue rate than the rest of the population. SDH volume and the absence of focal deficit at presentation are the two strongest individual contributors. External validation is required before clinical adoption.</p>
